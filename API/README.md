...
Este backend agora vive dentro da pasta `API/` do workspace.
...
Execute os comandos abaixo a partir da pasta `API/`.
...
- A API agora tenta carregar automaticamente `API/.env`.
- Se ele ainda nao existir em desenvolvimento local, ela usa `API/.env.example` como fallback de valores basicos.
...
- A senha inicial do membro passa a ser o `CPF` dele por padrao. Opcionalmente, o cadastro inicial tambem pode usar `1234` como senha temporaria.
- Sempre que o membro autenticar usando `CPF` ou `1234`, a API marca a sessao com `mustChangePassword` e bloqueia as areas protegidas ate a troca da senha.
- A nova senha definitiva deve ter pelo menos `6` caracteres, com ao menos `1` letra e `1` numero.
